...
import { Bell, CheckCircle2, X, Trash2, LogIn, UserPlus } from 'lucide-react'
...
export function Header() {
  ...
  const { token, isDemoMode } = useAuthStore()
  ...
  const handleNotificationClick = (id: string, analysisId?: number) => {
    markRead(id)
  ...
  return (
    <header className="flex items-center justify-between px-8 py-5 border-b border-white/[0.07] bg-black/90 backdrop-blur-md sticky top-0 z-10">
      ...
      <div className="flex items-center gap-2">
        {/* Guest / demo: show Login + Sign Up */}
        {!isRealUser && (
          <div className="flex items-center gap-2">
            <Link
              to="/login"
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm font-medium text-slate-300 hover:text-slate-100 hover:bg-white/[0.06] border border-white/[0.08] hover:border-white/[0.14] transition-all"
            >
              <LogIn size={14} />
              Log in
            </Link>
            <Link
              to="/signup"
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm font-medium bg-violet-600 hover:bg-violet-500 text-white transition-all shadow-lg shadow-violet-500/20"
            >
              <UserPlus size={14} />
              Sign up
            </Link>
        ...
        {/* Notification bell — always visible when authenticated (real or demo) */}
        {token && (
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((n) => (
                        <div
                          ...
                          onClick={() => handleNotificationClick(n.id, n.analysisId)}
                        >
                          {!n.read && (
